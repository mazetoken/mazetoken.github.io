...
contract NFTMarket is Ownable {
	...
	/// @dev Set and update fee address
	/// @dev Set and update fee amount

	address public feeAddress;
	uint256 public feeAmount;

	constructor(address fee_Address, uint256 fee_Amount) {
    //feeAddress = msg.sender;
	feeAddress = fee_Address; //feeAddress = 0x...;
	feeAmount = fee_Amount; //feeAmount = 100000000000000; // 0.00010000 BCH
  	}
	...
	function updateFee(uint256 _newfeeAmount) external onlyOwner {
	...
	function list(ERC721 tokenContract, uint256 tokenId, uint256 askPrice) external payable returns (uint256) {
	...
	function cancelListing(uint256 listingId) external payable {
